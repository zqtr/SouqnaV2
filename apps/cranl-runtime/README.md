...
- Provider abstractions for OpenAI, Ollama, Hugging Face, and Fanar.
- Fanar chat through the private RunPod/vLLM OpenAI-compatible endpoint.
...
4. Set provider credentials such as `OPENAI_API_KEY`, `OLLAMA_URL`, and the Fanar endpoint values.
   For Fanar chat, set `FANAR_API_URL` to the RunPod/vLLM `/v1` URL and `FANAR_API_KEY` to the same key passed to `vllm serve --api-key`.
...
HUGGINGFACE_CHAT_MODEL=QCRI/Fanar-2-27B-Instruct
HUGGINGFACE_ALLOWED_CHAT_MODELS=QCRI/Fanar-2-27B-Instruct,QCRI/Fanar-1-9B-Instruct
...
CranL defaults `ai-chat` jobs to the Fanar provider and uses
`QCRI/Fanar-1-9B-Instruct` unless the job supplies another Fanar model.

Fanar chat requires a vLLM endpoint that supports the OpenAI-compatible
`/chat/completions` route. Configure `FANAR_API_URL` with the base URL ending in
`/v1`; CranL appends `/chat/completions`. Keep `FANAR_API_KEY` only in server-side
runtime environments and never expose it to the browser.
