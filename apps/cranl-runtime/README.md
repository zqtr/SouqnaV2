...
CranL expects Redis to be available at `REDIS_URL`. Run Redis locally with Docker:
...
`CRANL_DEFAULT_PROVIDER` controls which provider handles AI jobs when a request
does not specify one. Supported values are `openai`, `ollama`, `huggingface`,
and `mock`. Use `mock` for local queue/UI testing without external model
credentials; use `openai`, `ollama`, or `huggingface` only when the matching
credential or local service is configured.
...
The Souqna web app reaches these through `/api/cranl/*` after `CRANL_RUNTIME_URL`
and `CRANL_API_KEY` are set in the frontend environment.
...
4. Set `CRANL_DEFAULT_PROVIDER` and provider credentials such as `OPENAI_API_KEY`, `OLLAMA_URL`, and `HUGGINGFACE_API_KEY`.
