...
jobsRouter.post('/jobs/image-generation', async (req, res, next) => {
  try {
    const data = ImageGenerationJobSchema.parse(withDefaultProvider(req.body, 'openai'));
...
jobsRouter.post('/jobs/ai-chat', async (req, res, next) => {
  try {
    const data = AiChatJobSchema.parse(withDefaultProvider(req.body, 'fanar'));
