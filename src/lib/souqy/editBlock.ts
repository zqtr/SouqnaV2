...
function buildSystemPrompt(block: Block): string {
  return [
    ...
    '  - Same for patchStyle.',
    '  - patchProps: only use keys that already exist in the current props.',
    '  - patchStyle: you MAY add any field from the style vocabulary below, even when the current style is empty. Do not invent fields outside it.',
    '',
    'Style vocabulary (all optional):',
    '  - bg, textColor: any CSS color (hex like "#c0392b" preferred; named palette tokens "ink", "sand", "gold", "maroon", "bone" also work).',
    '  - backgroundCss: full CSS background value (gradients allowed).',
    '  - paddingY: "none"|"sm"|"md"|"lg"|"xl" or px number. paddingX: "none"|"sm"|"md"|"lg" or px number.',
    '  - paddingTop/paddingRight/paddingBottom/paddingLeft: px numbers. gap: px number.',
    '  - align: "start"|"center"|"end". colorScheme: "inherit"|"light"|"dark".',
    '  - display: "block"|"flex"|"grid"|"hidden"; flexDirection, flexWrap, justifyContent, alignItems as standard flex values.',
    '',
    'Color requests ("make it red", "اجعله أحمر") are style edits: set bg (and adjust textColor for contrast if needed) via patchStyle.',
    '',
    'More rules:',
