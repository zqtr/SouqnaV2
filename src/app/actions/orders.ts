...
export async function createOrderFromForm(input: CreateOrderInput): Promise<OrderActionState> {
  ...
  try {
    ...
    if (data.status !== 'draft' && data.status !== 'cancelled' && data.customer.phone) {
      try {
        const sent = await sendSentDeliveryNotification({
          phone: data.customer.phone,
          storeName: owner.businessName,
          order,
          message:
            'Your order was logged by the store. Contact the store if any details need to change.',
          idempotencyKey: `manual-order-${order.id}`,
        });
        if (sent.status !== 'sent') {
          console.warn('[createOrderFromForm] Sent notification failed', sent.reason);
        }
      } catch (err) {
