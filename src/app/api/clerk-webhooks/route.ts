...
function primaryEmailFromClerkUser(data: ClerkUserPayload): string | null {
  const emails = data.email_addresses ?? [];
  const primary = emails.find((email) => email.id === data.primary_email_address_id);
  return primary?.email_address?.trim() || emails[0]?.email_address?.trim() || null;
}

function usernameFromClerkUser(data: ClerkUserPayload): string | null {
  const username = data.username?.trim().toLowerCase();
  return username || null;
}

async function upsertAccountProfile(data: ClerkUserPayload): Promise<void> {
  if (!hasDb() || !data.id) return;
  const username = usernameFromClerkUser(data);
  const displayName = ownerNameFromClerkUser(data);
  const primaryEmail = primaryEmailFromClerkUser(data);
  await db()`
    insert into account_profiles (
      clerk_user_id, username, display_name, primary_email, updated_at
    )
    values (${data.id}, ${username}, ${displayName}, ${primaryEmail}, now())
...
async function onUserCreated(data: ClerkUserPayload): Promise<void> {
  ...
  if (!hasDb()) return;
  await upsertAccountProfile(data);
...
async function onUserUpdated(data: ClerkUserPayload): Promise<void> {
  ...
  await patchPlanMeta(clerkUserId, {
    ...(notificationPhone.phone
      ...
      : {}),
    founder_name: founderName,
    username: usernameFromClerkUser(data),
  });
  try {
    await maybeSendAccountWelcomeEmail(data);
  } catch (err) {
...
async function onUserDeleted(clerkUserId: string): Promise<void> {
  ...
  await db()`delete from account_profiles where clerk_user_id = ${clerkUserId}`;
}
