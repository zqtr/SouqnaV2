...
import { COMPONENT_LIBRARY_GROUPS } from '@/lib/blocks/componentCatalog';
import {
  COMPONENT_SHOWCASE_SLUG,
  hasLegacyComponentShowcaseAssets,
  normalizeComponentShowcaseBlocks,
} from '@/lib/blocks/componentShowcaseAssets';
import type {
  Block,
  BLOCK_TYPES,
  BlockVariant,
...
const LIBRARY_GROUPS = COMPONENT_LIBRARY_GROUPS;
  ...
  switch (type) {
    ...
    case 'drop':
      return (
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: isDrag ? 5 : 2 }}>
            <div style={{ height: isDrag ? 4 : 1.4, width: '72%', background: sand }} />
            <div style={{ height: isDrag ? 2 : 1, width: '88%', background: sandSoft }} />
            <div style={{ height: isDrag ? 2 : 1, width: '56%', background: sandSoft }} />
          </div>
          <div
            style={{
              width: isDrag ? 30 : 11,
...
function BuilderShellInner({
  ...
  const isComponentShowcaseBuilder = slug.trim().toLowerCase() === COMPONENT_SHOWCASE_SLUG;
  const initialBlocksForStore = useMemo(
    () =>
      isComponentShowcaseBuilder ? normalizeComponentShowcaseBlocks(initialBlocks) : initialBlocks,
    [initialBlocks, isComponentShowcaseBuilder],
  );
  ...
  const [blocks, setBlocks] = useState<Block[]>(initialBlocksForStore);
  const [selectedId, setSelectedId] = useState<string | null>(initialBlocksForStore[0]?.id ?? null);
  ...
  const persist = useCallback(
    async (next: Block[]) => {
      setSaveState('saving');
      const normalized = isComponentShowcaseBuilder ? normalizeComponentShowcaseBlocks(next) : next;
      const cleaned = sanitizeBlocksForSave(normalized);
    ...
    [activePageId, isComponentShowcaseBuilder, slug],
  );
  ...
  useEffect(() => {
    if (!isComponentShowcaseBuilder) return;
    const currentBlocks = blocksRef.current;
    if (!hasLegacyComponentShowcaseAssets(currentBlocks)) return;
    const normalized = normalizeComponentShowcaseBlocks(currentBlocks);
    setBlocks(normalized);
    setIframeKey((key) => key + 1);
    queuePersist(normalized);
  }, [isComponentShowcaseBuilder, queuePersist]);
  ...
  const update = useCallback(
    (next: Block[]) => {
      const nextForStore = isComponentShowcaseBuilder
        ? normalizeComponentShowcaseBlocks(next)
        : next;
      ...
      setBlocks(nextForStore);
      queuePersist(nextForStore);
    },
    [isComponentShowcaseBuilder, queuePersist],
  );
