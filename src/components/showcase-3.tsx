...
class Showcase3CanvasBoundary extends Component<
...
> {
  ...
  override componentDidCatch(error: Error, info: ErrorInfo) {
    console.warn('[Showcase3] Canvas failed; falling back to static cards.', error, info);
  }
...
function StaticShowcaseFallback({
  ...
  return (
    <div className="flex h-full w-full items-center justify-center gap-3 overflow-x-auto px-2 py-3">
      {items.map((item, index) => {
        ...
        return (
          <button
            ...
            className={`relative h-full min-h-[180px] w-[150px] shrink-0 overflow-hidden rounded-[18px] border bg-neutral-100 text-left shadow-sm transition sm:w-[190px] ${
              isActive
          ...
          >
            ...
            <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent px-3 pb-3 pt-12 text-white">
              <span className="block text-xs uppercase tracking-[0.14em] text-white/75">
                {item.category}
              </span>
...
export function Showcase3({
  ...
  const canvasResetKey = normalizedItems.map((item) => item.imageUrl).join('|');
